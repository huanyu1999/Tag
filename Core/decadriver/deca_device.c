...
void dwt_configure(dwt_config_t *config)
{
    ...
    uint8 prfIndex = config->prf - DWT_PRF_16M;  //如PRF=16M，则prfIndex=0    如PRF=64M，则prfIndex=1
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics)
{
    ...
    diagnostics->rxPower = 0;
    diagnostics->fpPower = 0;
    diagnostics->quality = 0;
    ...
    //double A  = 113.77; // for PRF of 16 MHz (see pg. 45 of DW1000 user manual)
    ...
    diagnostics->fpPower = 10.0 * log10((pow(F1, 2.0) + pow(F2, 2.0) + pow(F3, 2.0)) / pow(N, 2.0)) - A;
	diagnostics->rxPower = 10.0 * log10((C * pow(2.0, 17.0))/ pow(N, 2.0)) - A;
    diagnostics->quality = (double)(1.0 * diagnostics->firstPathAmp2) / (1.0 * diagnostics->stdNoise);
 ...
 * no return value
 */
void _dwt_configlde(int prfIndex)  //如PRF=16M，则prfIndex=0    如PRF=64M，则prfIndex=1
{
